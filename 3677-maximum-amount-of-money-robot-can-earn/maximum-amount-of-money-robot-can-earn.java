class Solution {
    private int solve(int i, int j, int k, int[][] coins, int[][][] dp) {
        ...
        if( k > 2 )return Integer.MAX_VALUE;
        if (i >= n || j >= m)return 0;
        ...
        if (j + 1 < m) {
            int take = solve(i , j+1 , k , coins , dp ) + coins[i][j];
            int skip = Integer.MIN_VALUE;

            if (coins[i][j] < 0 && k < 2) skip = solve(i , j+1 , k+1 , coins , dp );
        ...
        if (i + 1 < n) {
            int take = solve(i+1 , j , k , coins , dp ) + coins[i][j];
            int skip = Integer.MIN_VALUE;

            if (coins[i][j] < 0 && k < 2) skip = skip = solve(i+1 , j , k+1 , coins , dp );
